...
if st.session_state.participant_id == "":
    ...
    with st.form(key="prompt_form", clear_on_submit=True):
        user_input = st.text_area(
            "在这里输入你的提示词 (Prompt)：",
            height=100,
            key="prompt_input"  # 可选，但建议保留以方便读取
        )
        st.markdown("👇 **请点击以下按钮提交你的提示词（请选择最符合你当前意图的行为）：**")
        col1, col2, col3, col4, col5 = st.columns(5)
        # 用 form_submit_button 替代普通 button
        ...
        if clicked_behavior:
            if user_input.strip() == "":
                ...
                # 表单会清空，但为了用户体验，我们不让它提交成功，但无法停止提交，所以只能显示警告后 st.stop
                st.stop()
            else:
                # 处理正常提交
                # 显示用户消息
                ...
                # 表单的 clear_on_submit=True 会自动清空所有组件，包括 text_area
                # 但我们还需手动清除 key 对应的 session_state 值，以免残留
                # 注意：由于 clear_on_submit 会重置组件，但 session_state 中的 key 值不会被自动清除，我们需要手动清除
                # 但是，因为表单提交后脚本会 rerun，新的脚本执行时，text_area 的 key 会重新初始化，
                # 但如果我们不清除 session_state["prompt_input"]，新脚本执行时，该值可能仍然是旧值，
                # 导致 text_area 显示旧内容。所以我们需要在提交后清除该键。
                # 注意：使用 clear_on_submit 时，组件值会被重置，但 session_state 中的值不会被重置，
                # 所以我们需要在提交后手动重置 session_state["prompt_input"]。
                # 但为了安全，我们使用 try-except 包裹。
                try:
