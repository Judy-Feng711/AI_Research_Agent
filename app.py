...
def load_participant_state(pid):
    ...
    try:
        ...
        for log in log_data:
            if (
                log.get("behavior_button") in valid_behaviors
                and log.get("user_prompt")
                and log.get("user_prompt").strip() != ""
            ):
        ...
        # 3. 从 participant_state 加载并比较
        state_resp = (
            supabase.table("participant_state")
            .select("*")
            .eq("participant_id", pid)
            .execute()
        )
        ...
        if state_resp.data:
            raw = (
                json.loads(state_resp.data[0]["messages"])
                if state_resp.data[0]["messages"]
                else []
            )
...
def save_plan(
    ...
    try:
        ...
        st.error(
            f"方案保存失败：{e} "
            f"请确保数据库表已添加 "
            f"task4_text, task5_text, task6_text 列。"
        )
        return False
...
if (
    "mode" in query_params
    and query_params["mode"] == "admin"
):
...
st.markdown(
    ...
    <style>
        ...
        .st-key-main_row
        [data-testid="stHorizontalBlock"]
        > div.stColumn:first-child h3 {
            color: #1565c0 !important;
        }

        .st-key-main_row
        [data-testid="stHorizontalBlock"]
        > div.stColumn:last-child h3 {
        ...
        /* 子任务输入框背景色 */
        .st-key-task1_text textarea {
            background-color: #e6f3ff;
        }

        .st-key-task2_text textarea {
            background-color: #f5e6ff;
        }

        .st-key-task3_text textarea {
            background-color: #e6f3ff;
        }

        .st-key-task4_text textarea {
            background-color: #f5e6ff;
        }

        .st-key-task5_text textarea {
            background-color: #e6f3ff;
        }

        .st-key-task6_text textarea {
            background-color: #f5e6ff;
        }
    </style>
    """,
...
st.markdown(
    ...
    <div style="text-align: center;">
        <h1 style="font-size: 36px; margin-bottom: 0;">
            🎓 教育实证研究全周期智能协同框架
        </h1>
        <p style="font-size: 20px; color: #555; margin-top: 4px;">
            Intelligent Collaborative Framework for Empirical Research in Education (ICFER)
        </p>
    </div>
    """,
    unsafe_allow_html=True
...
if st.session_state.user_role == "研究者":
    ...
    with col_center:
        st.markdown(
            "<h3 style='text-align: center;'>📊 研究者数据导出</h3>",
            unsafe_allow_html=True
        )
        ...
        if not st.session_state.export_authorized:
            ...
            try:
                ...
                if response.data:
                    ...
                    # 父级中心区占页面约 1/2，
                    # 内部 4/18，使按钮宽度约等于整页 1/9
                    dl_logs_left, dl_logs_center, dl_logs_right = st.columns(
                        [7, 4, 7]
                    )
            ...
            try:
                ...
                if response_plan.data:
                    ...
                    dl_plans_left, dl_plans_center, dl_plans_right = st.columns(
                        [7, 4, 7]
                    )

                    with dl_plans_center:
                        st.download_button(
                            label="📥 下载方案数据",
            ...
            try:
                ...
                if response_consent.data:
                    df_consent = pd.DataFrame(
                        response_consent.data
                    )

                    csv_consent = df_consent.to_csv(
                        index=False,
                        encoding="utf-8-sig"
                    )

                    dl_consent_left, dl_consent_center, dl_consent_right = st.columns(
                        [7, 4, 7]
                    )

                    with dl_consent_center:
                        st.download_button(
                            label="📥 下载知情同意记录",
            ...
            # 退出研究者模式
            admin_exit_left, admin_exit_center, admin_exit_right = st.columns(
                [7, 4, 7]
            )

            with admin_exit_center:
                if st.button(
                    "退出研究者模式",
                    key="exit_admin_mode",
                    use_container_width=True
    ...
    if st.session_state.experiment_completed:
        ...
        col_btn_left, col_btn_center, col_btn_right = st.columns(
            [1, 1, 1]
        )
    ...
    if not st.session_state.participant_id:
        ...
        col_space1, col_id, col_space2 = st.columns(
            [3, 1, 3]
        )
    ...
    if not st.session_state.consent_given:
        st.markdown(
            ...
            f"当前参与者编号："
            f"<strong>{st.session_state.participant_id}</strong>"
            "</p>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            ...
            <div class="consent-card">
                ...
                <p style="text-align:center; color:#888; font-size:13px; margin-top:-10px;">
                    版本号：v1.0_ICFER_2026　|　生效日期：2026-09-20
                </p>

                <p>
                    <strong>研究主题：人工智能辅助教育研究的特征与机制研究</strong>
                </p>

                <p>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    您已完成本研究的问卷阶段。本页为研究第二阶段的补充知情说明，
                    请您阅读后决定是否继续参与人机交互任务。
                </p>
                ...
                <p>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    尊敬的参与者，您好！我们是陕西师范大学教育学部的科研团队，
                    诚挚地邀请您参与我们的研究项目。在您点击"同意"按钮之前，
                    请务必仔细阅读以下内容，以确保您充分了解本研究的目的、流程、
                    潜在风险与收益，以及您的各项权利。如有任何疑问，
                    欢迎随时与我们联系。
                </p>
                ...
                <p>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    本研究致力于探索教育学及相关专业的硕士、博士研究生在实际科研工作中
                    如何与生成式人工智能（AI）协同工作。我们将通过观察您与 AI 共同完成
                    一项研究设计任务的过程，来分析您的行为模式、思维过程及主观感受。
                    最终，本研究的成果将有助于制定更负责任、更可解释的 AI 使用指南，
                    为高校和相关机构的科研培训提供依据。
                </p>

                <p>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <strong>本次实验的具体任务</strong>：
                    您将与我们的智能研究助理
                    <strong>
                        ICFER（教育实证研究全周期智能协同框架，
                        Intelligent Collaborative Framework for Empirical Research in Education）
                    </strong>
                    进行大约 <strong>100 分钟</strong> 的深度对话。
                </p>

                <p>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    在对话中，您将围绕统一主题
                    <strong>"人工智能时代的教师教育与教师专业发展研究"</strong>，
                    结合您自身的学科专长（如学科教学、教育技术、教育管理等），
                    从中选定一个具体的研究切入点，并在 ICFER 的辅助下构思并完成
                    一份完整的实证研究设计方案。
                </p>
                ...
                <p>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <em>特别说明</em>：
                    这项设计任务是实验环节中的一次模拟任务，您所完成的设计方案
                    仅用于本研究分析，不会用于课程评价、科研考核、职称评定
                    或真实学术成果提交。实验中使用的材料均为统一编撰并经过脱敏处理，
                    不要求您提交个人真实论文、未公开数据、评审材料或其他涉及个人、
                    单位及科研保密的信息。
                </p>
                ...
                <ul>
                    <li>
                        <strong>深度人机对话</strong>：
                        您将与 ICFER 围绕上述统一主题下您所聚焦的具体研究问题
                        进行自由、深入的探讨。ICFER 会尝试辅助您梳理思路、
                        提供建议、完善研究设计。
                    </li>

                    <li>
                        <strong>方案构思与生成</strong>：
                        在 ICFER 的辅助下，您需要在系统中逐步填写研究设计的关键要素
                        （如研究问题、研究方法、数据收集与分析计划等），
                        最终形成一份属于您自己的研究方案初稿。
                    </li>

                    <li>
                        <strong>数据自动记录</strong>：
                        为了科学研究，您与 ICFER 的
                        <strong>完整对话日志</strong>，
                        以及您在各阶段填写的研究要点将被系统自动记录，
                        这些信息共同构成您的研究方案记录。
                    </li>
                </ul>
                ...
                <p>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    我们深知您学术成果与个人隐私的重要性，并承诺采取最高标准的保护措施。
                    所有数据将严格遵循科研伦理规范进行处理：
                </p>
                ...
                <ul>
                    <li>
                        <strong>用途限定</strong>：
                        所有收集的数据<strong>仅用于本项目的学术分析</strong>，
                        不会用于任何商业目的，也不会被用于训练 AI 模型。
                    </li>

                    <li>
                        <strong>匿名化处理（去标识化）</strong>：
                        在分析前，所有数据（包括对话日志）都将进行严格的匿名化处理，
                        即删除您的姓名、联系方式、单位等可直接识别您身份的信息。
                        最终公开发表的研究成果中，仅会呈现无法识别个体的引语或任务示例。
                    </li>

                    <li>
                        <strong>安全存储</strong>：
                        原始电子数据将加密存储于受权限控制的学校服务器或加密存储设备中。
                    </li>

                    <li>
                        <strong>信息保密与销毁</strong>：
                        我们承诺，未经您的单独同意，绝不会向任何合作单位、
                        AI 平台供应商或无关第三方提供可识别您的数据。
                        所有原始数据将在研究完成后（预计<strong>3 年内</strong>）
                        进行彻底销毁。
                    </li>

                    <li>
                        <strong>学术诚信保护</strong>：
                        本项目的研究结果<strong>不会</strong>用于任何形式的个人能力排名、
                        心理诊断、学术不端判定、导师评价或职称考核。
                    </li>

                    <li>
                        <strong>对话内容的编码分析</strong>：
                        您与 ICFER 的对话文本，将在数据收集完成后，
                        由研究人员依据国际通用认知框架进行编码分析，
                        用于识别对话的认知层次与协同模式。
                        编码仅针对对话内容本身，不针对您个人做任何能力评价。
                    </li>

                    <li>
                        <strong>与前期数据的关联</strong>：
                        您在本阶段获得的编号，将用于将您的对话数据与您此前参与研究时
                        产生的数据进行关联分析。您报名时留下的联系方式仅用于研究协调与邀请，
                        不会与您的对话数据一并存储。在数据分析开始前，
                        联系方式将被删除，仅保留随机编号。
                    </li>
                </ul>
                ...
                <ul>
                    <li>
                        ...
                        <ul>
                            ...
                            <li>
                                <strong>心理风险</strong>：
                                您可能会在接触 AI 不完美的回答、或反思自身对 AI 的依赖时，
                                产生短暂的紧张、挫败或疲劳感。
                                研究团队的任务设计采用中性、非评判性表述，
                                研究人员不会对您的表现做任何负面评价。
                                若您出现明显不适，我们将立即提供休息和解释。
                            </li>

                            <li>
                                <strong>信息安全风险</strong>：
                                为保障您的科研信息安全，请<strong>勿</strong>在研究过程中
                                向 AI 输入您未发表的论文正文、未公开数据、评审材料
                                或带有单位信息的保密内容。确需使用 AI 平台时，
                                请避免输入真实姓名、联系方式、未公开研究成果、
                                个人敏感信息、保密数据或具有识别性的他人资料。
                            </li>

                            <li>
                                <strong>身体风险</strong>：
                                主要为长时间使用电脑带来的常规用眼疲劳或久坐不适，
                                建议您适时休息。
                            </li>
                        </ul>
                    </li>
                    ...
                    <li>
                        <strong>您的潜在收益</strong>：

                        <ul>
                            <li>
                                您将获得一次深度的、前沿的 AI 辅助科研体验。
                            </li>

                            <li>
                                您将免费获得一份由您与 ICFER 共同完成的、
                                <strong>为您量身定制的研究设计方案初稿</strong>，
                                这对您未来的研究可能具有参考价值。
                            </li>
                        </ul>
                    </li>
                ...
                <ul>
                    <li>
                        在实验过程中，如果您感到任何不适或希望终止，
                        可随时点击界面正下方的<strong>"退出实验"</strong>按钮。
                    </li>

                    <li>
                        退出后，系统将立即停止数据记录。
                        您已产生的对话日志将不再纳入后续数据分析。
                        若您希望删除已记录的数据，可通过研究负责人邮箱提出，
                        我们将在数据匿名化前为您删除。退出不会对您产生任何不利影响。
                    </li>

                    <li>
                        如果数据已完成匿名化并进入汇总分析，
                        届时删除可能受限，这一点我们会在研究中提前说明。
                    </li>
                </ul>
                ...
                <p>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <strong>会的。</strong>
                    研究结束后，我们承诺在不泄露个人隐私的前提下，
                    通过电子邮件等方式向有需要的参与者分享一份总体研究发现摘要，
                    以及负责任使用 AI 的实践建议。
                    如您希望接收，可通过研究负责人邮箱与我们联系。
                </p>
    ...
    if st.session_state.show_exit_dialog:
        st.warning(
            "您确定要退出实验吗？退出后，您本次实验的所有数据"
            "将不会被纳入最终数据分析。"
        )

        # 两个按钮居中并列
        # 每个按钮占整页约 1/9，与底部“退出实验”按钮一致
        (
            confirm_space_left,
            col_confirm1,
            col_confirm2,
    ...
    if st.session_state.participant_id:
        ...
        with st.container(key="main_row"):
                ...
                with st.container():
                    with st.container(
                        ...
                        for msg in st.session_state.messages:
                            ...
                            if (
                                msg["role"] == "assistant"
                                and msg["content"] == INITIAL_GREETING
                            ):
                                continue
                    ...
                    with st.container():
                        with st.form(
                            ...
                            elif col_b2.form_submit_button(
                                "规范语言/格式",
                                use_container_width=True
                            ):
                                clicked_behavior = "规范语言/格式"

                            elif col_b3.form_submit_button(
                                "微调研究逻辑",
                                use_container_width=True
                            ):
                                clicked_behavior = "微调研究逻辑"

                            elif col_b4.form_submit_button(
                                "重构研究方案",
                                use_container_width=True
                            ):
                                clicked_behavior = "重构研究方案"

                            elif col_b5.form_submit_button(
                                "拓展研究思路",
                                use_container_width=True
                            ):
            ...
            with col_right:
                ...
                st.caption(
                    "任务共分为 6 个递进环节，请根据您与AI的完整对话，"
                    "将各环节的核心成果填入下方对应模块。"
                    "您可以在交互过程中随时记录，或最后集中整理。"
                )
        ...
        col_exit1, col_exit_center, col_exit2 = st.columns(
            [4, 1, 4]
        )
