...
1. **5 个行为按钮**：文字不再被截断（“获取基础…” “拓展研究…”），允许自动换行，高度自适应，视觉更整齐。
2. **提交方案按钮**：去掉了左边那条又长又空的白色圆角条，按钮现在干净地靠右显示，不再有多余空白。

只改了 CSS + 提交按钮那一行 Python 代码，其他所有逻辑、间距、卡片颜色都保持原样。

把下面这段完整代码直接替换即可（已包含所有之前的卡片背景、淡蓝/淡绿等效果）。
...
**修改说明（非常克制）：**
- 5个按钮现在允许文字换行，不会再出现“获取基础…”这种截断。
- 提交方案按钮去掉了左边那条空白色条，直接全宽显示，干净很多。
- 所有卡片背景、左右分栏、间距都完全没动。

刷新页面后效果会立刻变好。如果还有想微调的地方（比如按钮再小一点、再圆一点），随时告诉我。
